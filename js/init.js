    ...
    $("#canvas_container").droppable({
        drop: function (event, ui) {
            var svgXY = getSvgCoordinates(event, paper);
						alert("Window X: " + event.originalEvent.target.x + "Window Y: " + event.originalEvent.target.y + "SVG X: " + svgXY.x + "SVG Y: " + svgXY.y);
						...
						var newfighter = new Fighter(null, paper);
						paper.setStart();
						newfighter.drawElement();
						paper.setFinish();
						newfighter.el.attr({transform: ['t', svgXY.x, svgXY.y]});
						unitMouseupHandler(newfighter.el);
